...
class ParticleFilter(Node):
    ...
    def initialise_particles(self):
        ...
        for i in range(self.num_particles_):
            # Sample x, y, and theta uniformly within the map bounds
            ...
            # Equal weight for all particles
    ...
    def clicked_point_callback(self, clicked_point_msg):
        ...
        for i in range(self.num_particles_):
            # Sample x and y from Gaussian distribution around clicked point
            x = np.random.normal(point_x, self.clicked_point_std_dev_)
            y = np.random.normal(point_y, self.clicked_point_std_dev_)
            
            # Orientation should be uniform between 0 and 2*pi
            theta = random_uniform(0, 2 * math.pi)
    ...
    def normalise_weights(self):
        ...
        if total_weight > 0:
            # Normalize each particle's weight so the total sum equals 1.0
            for particle in self.particles_:
                particle.weight /= total_weight
        else:
            # If all weights are zero, set them to equal weights
    ...
    def estimate_pose(self):
        ...
        ## Task 6         ##
        ####################
        # Compute weighted average of x and y positions
        total_weight = sum(p.weight for p in self.particles_)
        
        if total_weight > 0:
            # Weighted average for x and y coordinates
            estimated_pose_x = sum(p.x * p.weight for p in self.particles_) / total_weight
            estimated_pose_y = sum(p.y * p.weight for p in self.particles_) / total_weight
            
            # For theta, we need to handle the circular nature of angles
            # Use complex number representation to average angles properly
            cos_sum = sum(math.cos(p.theta) * p.weight for p in self.particles_)
            sin_sum = sum(math.sin(p.theta) * p.weight for p in self.particles_)
            
            # Convert back to angle
            estimated_pose_theta = math.atan2(sin_sum / total_weight, cos_sum / total_weight)
            estimated_pose_theta = wrap_angle(estimated_pose_theta)
        else:
            # Fallback: use particle with highest weight if total weight is zero
            if self.particles_:
                best_particle = max(self.particles_, key=lambda p: p.weight)
                estimated_pose_x = best_particle.x
                estimated_pose_y = best_particle.y
                estimated_pose_theta = best_particle.theta
    ...
    def odom_callback(self, odom_msg):
        ...
        for particle in self.particles_:
            ...
            noisy_distance = distance + distance_noise
            ...
            # Add rotation noise and update particle orientation
    ...
    def terrain_callback(self, terrain_msg):
        ...
        if self.use_terrain_:
            ...
            for p in self.particles_:
                ...
                # likelihood = ??
                ###################
                # Get the observed terrain class from the message
                observed_terrain_class = terrain_msg.data
                
                # Get the ground truth terrain class at this particle's location
                ground_truth_class = self.visual_terrain_map_.get_ground_truth(p.x, p.y)
                
                # Get the likelihood from the confusion matrix
                # P(z = observed_class | y = ground_truth_class)
                likelihood = self.visual_terrain_map_.confusion_matrix[ground_truth_class, observed_terrain_class]
        ...
        if self.use_laser_:
            ...
            for p in self.particles_:
                ...
                for i in range(0, len(scan_msg.ranges), step): 
                    ...
                    ## Task 7         ##
                    ####################
                    # Calculate the likelihood of this ray using Gaussian model
                    # P(z_i | particle) = (1/√(2πσ²)) * exp(-((ẑ - z_i)²)/(2σ²))
                    
                    # Calculate the difference between expected and observed range
                    range_diff = particle_range - scan_range
                    
                    # Calculate the Gaussian likelihood
                    variance = math.pow(self.sensing_noise_stddev_, 2)
                    normalization_factor = 1.0 / math.sqrt(2 * math.pi * variance)
                    exponential_term = math.exp(-math.pow(range_diff, 2) / (2 * variance))
                    
                    ray_likelihood = normalization_factor * exponential_term
                    
                    # Multiply this ray's likelihood into the total likelihood
    ...
    def compass_fusion(self):
        ...
        # Instead of using compass as an observation update (likelihood-based), 
        # we can directly update particle orientations since compass provides 
        # absolute heading information. This is more effective because:
        # 1. Compass gives drift-free absolute orientation
        # 2. We can directly reduce orientation uncertainty
        # 3. More efficient than likelihood weighting for this type of sensor
        
        for particle in self.particles_:
            # Calculate the difference between particle orientation and compass reading
            angle_diff = wrap_angle(particle.theta - self.compass_)

            # Update particle weight based on how close its orientation is to compass reading
            # Use Gaussian likelihood: particles with orientations closer to compass get higher weights
            likelihood = math.exp(-0.5 * (angle_diff ** 2) / (self.magnetometer_noise_stddev_ ** 2))
